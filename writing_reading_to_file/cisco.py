...
def main():
    # print(os.getcwd()) # this return the current work directory
    ip_list = [
        '10.254.0.1',
        '10.254.0.2',
        '10.254.0.3',
    ]
    router_list = [] # it will contain instances of CiscoIOS
    for ip in ip_list:
        conn = CiscoIOS(ip, username='cisco', password='cisco')
        router_list.append(conn)
    for router in router_list: # make a directory for each router in the routers directory
        base_dir = os.getcwd() + '/routers/' 
        router_dir = base_dir + router.hostname + '/'
        os.mkdir(router_dir)
        data_dict = { # creating a dictionary 
            'running-config': router.get_run_cfg(), 
            'ip-interface': router.get_ip_int(), 
            'log': router.get_log(), 
        }
        for output in data_dict:  #creates a file name
            filename = router_dir + output + '.txt'
            with open(filename, 'w') as f: # opens filename and write output into it
