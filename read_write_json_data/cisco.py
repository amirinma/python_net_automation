...
import requests
import pprint
import json # You will use the json module to convert the Python dictionary containing lists, strings, and integers into a string containing proper JSON data.
...
class CiscoRestconf():
    ...
    def get_interface_info(self, data_type='json'):
        ...
        request_response = requests.get(uri, # making get request
            headers=headers, 
            auth=(self.username, self.password), 
            verify=False)
        return request_response.json()# return the JSON content of the request_response as a python data
    ...
    def get_interface_ips(self, interface_info, data_type='json'):
        ...
        return json.dumps(interface_info, indent=4) # return the value of interface_info, which is converted to a string of JSON data
