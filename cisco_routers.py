...
class ciscoIOS():
    ...
    def get_ip_int_br(self):
        """
        Return the router output from the command sh ip int br
        """
        return self.conn.send_command('sh ip int br')

    def get_ip_arp(self, as_dataframe=False ):
        """
        Return the router output from the command sh ip arp
        """
        if as_dataframe:
    ...
    def get_ip_route(self, connected=False):
        """ 
        Returns router output from the command sh ip route. If connected is True,  
        this function will only return data for directly connected IP routes 
        """
        if connected:
